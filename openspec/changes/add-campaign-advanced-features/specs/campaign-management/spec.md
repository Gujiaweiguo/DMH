...
系统 SHALL 支持更多表单字段类型，包括邮箱（email）、地址（address）、多行文本（textarea），以满足不同业务场景的需求。
...
### Requirement: 动态表单配置
系统 SHALL 支持动态配置活动报名表单，包括文本框、手机号、邮箱、地址、多行文本、下拉选择等字段类型，每个字段支持必填/选填配置和验证规则。

#### Scenario: 创建活动时配置表单
- **WHEN** 品牌管理员创建活动
- **AND** 在表单构建器中添加姓名（text）、手机号（phone）、邮箱（email）、地址（address）字段
- **AND** 设置姓名和手机号为必填，邮箱和地址为选填
- **THEN** 系统应保存表单配置到 campaigns.form_fields 字段
- **AND** 表单配置应包含字段类型、名称、标签、必填属性、验证规则

#### Scenario: 用户提交表单验证
- **WHEN** 用户在活动页面填写表单
- **AND** 提交表单数据
- **THEN** 系统应验证所有必填字段已填写
- **AND** 验证手机号格式（11 位数字，以 1 开头）
- **AND** 验证邮箱格式（符合邮箱正则表达式）
- **AND** 验证地址长度（5-200 字符）
- **AND** 验证失败时返回具体错误信息
...
### Requirement: 活动创建/编辑
系统 SHALL 支持创建和编辑活动，包括基础信息、动态表单配置、奖励规则、支付参数配置。
...
- **AND** 配置表单字段
- **AND** 设置奖励金额为 10.00 元
- **AND** 配置支付参数（订金 99.00 元，全款 299.00 元）
- **THEN** 系统应保存所有配置
- **AND** 返回创建成功的活动信息
- **AND** 活动状态为 active
...
### Requirement: 活动详情展示
系统 SHALL 在活动详情页展示活动的完整信息，包括基础信息、表单配置、奖励规则、支付配置、统计数据，并提供生成海报、查看支付二维码等操作入口。

#### Scenario: 查看活动详情包含支付配置
- **WHEN** 用户访问活动详情页
- **THEN** 系统应显示活动基本信息
- **AND** 显示动态表单配置
- **AND** 显示奖励规则
- **AND** 显示支付配置（订金/全款金额）
- **AND** 显示支付二维码
- **AND** 显示统计数据（参与人数、支付金额、奖励发放）

#### Scenario: 活动详情页生成海报入口
- **WHEN** 分销员访问活动详情页
- **THEN** 系统应显示"生成海报"按钮
- **AND** 点击按钮应跳转到海报生成页面
