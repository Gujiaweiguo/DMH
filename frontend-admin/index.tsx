...
const UserManagementView = defineComponent({
  setup() {
    const users = ref([
      ...
      { id: 2, username: 'brand_manager', realName: '品牌经理', role: '品牌管理员', status: 'ACTIVE', phone: '139****9999' },
...
const BrandManagementView = defineComponent({
  setup() {
    const brands = ref([
      { id: 1, name: '科技公司A', logo: 'https://api.dicebear.com/7.x/initials/svg?seed=TechA', status: 'ACTIVE', manager: 'brand_manager' },
      { id: 2, name: '教育机构B', logo: 'https://api.dicebear.com/7.x/initials/svg?seed=EduB', status: 'ACTIVE', manager: 'brand_manager' },
      { id: 3, name: '电商平台C', logo: 'https://api.dicebear.com/7.x/initials/svg?seed=EcomC', status: 'PAUSED', manager: 'brand_manager' }
    ]);
        ...
        h('div', { class: 'bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-lg transition-all' }, [
          h('div', { class: 'flex items-center gap-4 mb-4' }, [
            ...
            h('div', { class: 'flex-1' }, [
              ...
              h('p', { class: 'text-sm text-slate-500' }, `管理员: ${brand.manager}`)
            ])
          ]),
...
const CampaignManagementView = defineComponent({
  setup() {
    const campaigns = ref([
      { id: 1, name: 'Python编程训练营', startTime: '2024-01-15', endTime: '2024-02-15', status: 'ACTIVE', participants: 156 },
      { id: 2, name: 'AI人工智能课程', startTime: '2024-02-01', endTime: '2024-03-01', status: 'ACTIVE', participants: 89 },
      { id: 3, name: '数据分析实战班', startTime: '2024-01-01', endTime: '2024-01-31', status: 'PAUSED', participants: 234 }
    ]);
    ...
    // 打开内容编辑模态框
    const openContentModal = (campaign) => {
      editingCampaign.value = campaign;
      resetContentForm();
      contentForm.pageTitle = campaign.name;
      contentForm.pageDescription = campaign.description || '参与活动，获得丰厚奖励！';
          ...
          h('tbody', campaigns.value.map(campaign => h('tr', { class: 'border-b border-slate-50 last:border-0 hover:bg-slate-50/40' }, [
            ...
            h('td', { class: 'px-6 py-4' }, [
              h('div', { class: 'flex gap-2' }, [
                ...
                h('button', { 
                  onClick: () => openContentModal(campaign),
                  class: 'px-3 py-1 text-xs bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 font-bold' 
                }, '📝 页面设计'),
                h('button', { 
